		...
		<div class="flex flex-col gap-4">
			...
			<template v-if="!resetSent">
				<p class="text-c text-sm">
					Enter the email address connected to your account. We will send you a
					password reset link.
				</p>
				<div>
					<label
						for="resetEmail"
						class="block mb-1 text-b text-sm"
						>Email</label
					>
					<InputText
						v-model="resetEmail"
			...
			<template v-else>
				<div class="flex flex-col items-center text-center py-2">
					<div
						class="flex items-center justify-center w-14 h-14 rounded-full mb-4 bg-green-100 text-green-600 dark:bg-green-500/20 dark:text-green-400">
						<i class="pi pi-check text-2xl"></i>
					</div>
					<p class="text-b text-sm font-medium">Link sent!</p>
					<p class="text-c text-xs mt-1">
						Check your inbox at
						<span class="font-semibold">{{ resetEmail }}</span> and click the
...
:where(.my-app-dark, .my-app-dark *) .verify-banner {
	background: linear-gradient(
		135deg,
		color-mix(in srgb, var(--p-yellow-900) 30%, transparent),
		color-mix(in srgb, var(--p-orange-900) 20%, transparent)
	);
