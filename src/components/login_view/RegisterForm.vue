<template>
	<Dialog
		...
		<div
			class="flex flex-col items-center text-center animate-[fadeIn_0.4s_ease-out] relative z-10 pt-6">
			<div
			...
			<h2 class="mt-4 text-success text-xl md:text-2xl">
				{{ isUpgradeMode ? "Konto zabezpieczone!" : "Account created!" }}
			</h2>
			...
			<Button
				label="OK"
				class="mt-6 w-full surface-success text-contrast rounded-md py-2"
	...
	<h2
		v-if="!isUpgradeMode"
		class="text-center mb-6 text-a font-lora text-2xl">
		Join us 🌿
	</h2>
	...
	<form
		...
		<div>
			...
			<InputText
				...
				type="email"
				class="w-full"
				:class="!emailValid && touchedEmail ? 'border-red-500' : ''"
				placeholder="example@example.com"
				required />
			<p
				v-if="touchedEmail && debouncedEmail && !emailValid"
		...
		<div>
			...
			<InputText
				...
				id="password"
				class="w-full"
				:class="!passwordValid && touchedPassword ? 'border-red-500' : ''"
				placeholder="••••••"
				required />

			<Transition name="pwd-rules">
				<div
					v-if="touchedPassword && debouncedPassword && !passwordValid"
		...
		<div>
			...
			<InputText
				...
				class="w-full"
				:class="
					!passwordsMatch && touchedPasswordRepeat ? 'border-red-500' : ''
				"
				placeholder="••••••"
				required />
			<p
				v-if="
					touchedPasswordRepeat && debouncedPasswordRepeat && !passwordsMatch
				"
				class="text-danger text-sm mt-1">
		...
		<div class="flex flex-col text-sm mt-2">
			<Button
				:label="isUpgradeMode ? 'Create a free account' : 'Sign Up'"
				type="submit"
				class="w-full"
				:disabled="!formValid" />
			<AuthErrorBanner
				v-if="error !== '__email_not_verified__'"
				:error="error"
				@dismiss="error = null" />
		</div>
	</form>
	...
	<div
		v-if="!isUpgradeMode"
		class="my-6 flex items-center justify-center gap-4">
		<Button
			icon="pi pi-google"
			rounded
			outlined
...
@keyframes confettiDynamic {
	...
	100% {
		transform: translateY(240px) translateX(var(--shift-x))
			rotate(var(--rotate));
		opacity: 0;
	}
}
...
@media (max-width: 480px) {
	...
	@keyframes confettiDynamic {
		...
		100% {
			transform: translateY(180px) translateX(calc(var(--shift-x) * 0.8))
				rotate(var(--rotate));
			opacity: 0;
		}
	}
