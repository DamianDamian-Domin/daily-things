	...
	const initAuth = () => {
		return new Promise<void>(async (resolve) => {
			onAuthStateChanged(auth, async (firebaseUser) => {
				...
				// --- NASZ KROK 2: POBIERANIE PREFERENCJI ---
				if (firebaseUser) {
					const preferencesStore = usePreferencesStore();
					// Czekamy na pobranie ustawień, zanim powiemy aplikacji, że jest gotowa
					await preferencesStore.loadFromFirebase(firebaseUser.uid);
				}
				// -------------------------------------------
